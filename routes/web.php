<?php

use Illuminate\Support\Facades\Route;

Route::get('/', function () {
    $cards = [
        [ "title" => "Manutenção de Computadores",
        "content" => "Manutenção abrangente de hardware e software para PCs e laptops para garantir desempenho e longevidade ideais.",
        "svg" => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-wrench-icon lucide-wrench"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>',
        "src" => "/Gemini_Generated_Image_8hmzqi8hmzqi8hmz.png"],
        [ "title" => "Instalação de Servidores",
        "content" => "Instalação e configuração especializada de servidores locais e em nuvem, adaptados às necessidades do seu negócio.",
        "svg" => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-server-icon lucide-server"><rect width="20" height="8" x="2" y="2" rx="2" ry="2"/><rect width="20" height="8" x="2" y="14" rx="2" ry="2"/><line x1="6" x2="6.01" y1="6" y2="6"/><line x1="6" x2="6.01" y1="18" y2="18"/></svg>',
        "src" => "/Gemini_Generated_Image_5wmaie5wmaie5wma.png"],
        [ "title" => "Soluções em Firewall",
        "content" => "Implantação e gerenciamento de sistemas avançados de firewall para proteger sua rede contra ameaças cibernéticas em evolução.",
        "svg" => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shield-check-icon lucide-shield-check"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="m9 12 2 2 4-4"/></svg>',
        "src" => "/Gemini_Generated_Image_64awnq64awnq64aw.png"],
        [ "title" => "Configuração de rede",
        "content" => "Projeto, instalação e manutenção de infraestruturas de rede com e sem fio robustas e escaláveis.",
        "svg" => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-network-icon lucide-network"><rect x="16" y="16" width="6" height="6" rx="1"/><rect x="2" y="16" width="6" height="6" rx="1"/><rect x="9" y="2" width="6" height="6" rx="1"/><path d="M5 16v-3a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v3"/><path d="M12 12V8"/></svg>',
        "src" => "/Gemini_Generated_Image_4sri7q4sri7q4sri.png"],
        ["title" => "Venda de Computadores",
        "content" => "Instalação e configuração profissional de novos computadores desktop, estações de trabalho e periféricos para sua equipe.",
        "svg" => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-laptop-icon lucide-laptop"><path d="M18 5a2 2 0 0 1 2 2v8.526a2 2 0 0 0 .212.897l1.068 2.127a1 1 0 0 1-.9 1.45H3.62a1 1 0 0 1-.9-1.45l1.068-2.127A2 2 0 0 0 4 15.526V7a2 2 0 0 1 2-2z"/><path d="M20.054 15.987H3.946"/></svg>',
        "src" => "/Gemini_Generated_Image_pmuraspmuraspmur.png"]
    ];
    return view('welcome', ['cards' => $cards]);
});

Route::get('/contato', function(){

});
